...
# BACH SECTOR REUSE FUNCTIONS
#==============================

def graph_bach_sector(equips, tests, colors, data):
...
def sector_bach_info_view(request):
    if request.user.is_superuser or request.user.is_admin:
        qs_users = User.objects.filter(is_bach=True)

        context = {
            "title": "Infordmación de Tesistas",
            "qs_users": qs_users,
        }
        return render(request, 'labs/sectors/bach/info/bach_info_bachs.html', context)


def bach_equips_concrete_view(request):
    if request.user.is_superuser or request.user.is_admin:
        ...
        for name in qs_equips_names:
            ...
            qs_DiamondPiceBreak = Equip.objects.get(name=name).diamondpicebreak_set.filter(user__is_bach=True).count()
            qs_DiamondPice = Equip.objects.get(name=name).diamondpice_set.filter(diamond_pice_break__user__is_bach=True).count()
            qs_PiceBreak = Equip.objects.get(name=name).picebreak_set.filter(user__is_bach=True).count()
            qs_Pice = Equip.objects.get(name=name).pice_set.filter(pice_break__user__is_bach=True).count()
            qs_PrismBreak = Equip.objects.get(name=name).prismbreak_set.filter(user__is_bach=True).count()
            qs_Prism = Equip.objects.get(name=name).prism_set.filter(prism_break__user__is_bach=True).count()
        ...
        graph = graph_bach_sector(equips, tests, colors, data)
        script = graph[0]
        div = graph[1]
        ...
        return render(request, 'labs/sectors/bach/equips/bach_equips_concrete.html', context)


def bach_equips_material_view(request):
    if request.user.is_superuser or request.user.is_admin:
        qs_equips = Equip.objects.all()
        qs_equips_names = qs_equips.values_list("name", flat=True)
        qs_BrickType_equips = []
        ...
        for name in qs_equips_names:
            ...
            qs_BrickType = Equip.objects.get(name=name).bricktype_set.filter(user__is_bach=True).count()
            qs_VariationDimensions =Equip.objects.get(name=name).variationdimensions_set.filter(brick_type__user__is_bach=True).count()
            qs_Warping =Equip.objects.get(name=name).warping_set.filter(brick_type__user__is_bach=True).count()
            qs_DensityVoids =Equip.objects.get(name=name).densityvoids_set.filter(brick_type__user__is_bach=True).count()
            qs_Suction =Equip.objects.get(name=name).suction_set.filter(brick_type__user__is_bach=True).count()
            qs_AbsSatuCoeff =Equip.objects.get(name=name).abssatucoeff_set.filter(brick_type__user__is_bach=True).count()
            qs_CompretionBrick =Equip.objects.get(name=name).compretionbrick_set.filter(brick_type__user__is_bach=True).count()
            qs_WoodCompression = Equip.objects.get(name=name).woodcompression_set.filter(user__is_bach=True).count() 
            qs_ParallelPerpendicular =Equip.objects.get(name=name).parallelperpendicular_set.filter(wood_compression__user__is_bach=True).count()
            qs_MasonryCompression =Equip.objects.get(name=name).masonrycompression_set.filter(user__is_bach=True).count()
            qs_Masonry =Equip.objects.get(name=name).masonry_set.filter(masonry_compression__user__is_bach=True).count()
        ...
        return render(request, 'labs/sectors/bach/equips/bach_equips_material.html', context)


def bach_equips_soil_view(request):
    if request.user.is_superuser or request.user.is_admin:
        qs_equips = Equip.objects.all()
        qs_equips_names = qs_equips.values_list("name", flat=True)
        ...
        for name in qs_equips_names:
            ...
            qs_Equivalent = Equip.objects.get(name=name).equivalent_set.filter(user__is_bach=True).count()
            qs_Equiv = Equip.objects.get(name=name).equiv_set.filter(equivalent__user__is_bach=True).count()
            qs_FineMaterial = Equip.objects.get(name=name).finematerial_set.filter(user__is_bach=True).count()
            qs_GranulometricGlobal = Equip.objects.get(name=name).granulometricglobal_set.filter(user__is_bach=True).count()
            qs_GlobalMesh = Equip.objects.get(name=name).globalmesh_set.filter(granulometric_global__user__is_bach=True).count()
            qs_Limit = Equip.objects.get(name=name).limit_set.filter(user__is_bach=True).count()
            qs_Liquid = Equip.objects.get(name=name).liquid_set.filter(limit__user__is_bach=True).count()
            qs_Platic = Equip.objects.get(name=name).platic_set.filter(limit__user__is_bach=True).count()
            qs_MoistureContent = Equip.objects.get(name=name).moisturecontent_set.filter(user__is_bach=True).count()
            qs_MoistureMaterial = Equip.objects.get(name=name).moisturematerial_set.filter(moisture_content__user__is_bach=True).count()
            qs_ProctorM = Equip.objects.get(name=name).proctorm_set.filter(user__is_bach=True).count()
            qs_DensityWetDry = Equip.objects.get(name=name).densitywetdry_set.filter(proctor_m__user__is_bach=True).count()
            qs_Saturation = Equip.objects.get(name=name).saturation_set.filter(proctor_m__user__is_bach=True).count()
            qs_Correction = Equip.objects.get(name=name).correction_set.filter(proctor_m__user__is_bach=True).count()
            qs_SandCone = Equip.objects.get(name=name).sandcone_set.filter(user__is_bach=True).count()
            qs_HumidDensity = Equip.objects.get(name=name).humiddensity_set.filter(sand_cone__user__is_bach=True).count()
            qs_ContentMoisture = Equip.objects.get(name=name).contentmoisture_set.filter(sand_cone__user__is_bach=True).count()
            qs_ContentMoistureCarbure = Equip.objects.get(name=name).contentmoisturecarbure_set.filter(sand_cone__user__is_bach=True).count()
            qs_CorrectionSandCone = Equip.objects.get(name=name).correctionsandcone_set.filter(sand_cone__user__is_bach=True).count()
            qs_SpecificGravity = Equip.objects.get(name=name).specificgravity_set.filter(user__is_bach=True).count()
            qs_FractionPass = Equip.objects.get(name=name).fractionpass_set.filter(specific_gravity__user__is_bach=True).count()
            qs_FractionRetained = Equip.objects.get(name=name).fractionretained_set.filter(specific_gravity__user__is_bach=True).count()
        ...
        return render(request, 'labs/sectors/bach/equips/bach_equips_soil.html', context)
